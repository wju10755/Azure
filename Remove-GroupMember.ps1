...
# Title Screen
Write-Output '------------------------------------------------------------------------------------------------------------------'
Write-Host "<span style=`"color:red;`">                                 EasyJob - Schedule Group Member Removal </span>"
Write-Output '------------------------------------------------------------------------------------------------------------------'
Write-Output " "
...
Write-Host "<span style=`"color:#f6ff00;`">===Powershell Module Verification===</span>"
...
If (Get-Module -ListAvailable -name Az.Accounts) {
    ...
    Write-Host "<span style=`"color:#ff6700;`">Az.Accounts Module not found!</span>"
    Write-Output "Installing Az.Accounts Module, please wait..."
    Install-Module -Name Az.Accounts -Scope CurrentUser -AllowClobber 
    Write-Output " "
}

Write-Host "<span style=`"color:#f6ff00;`">===Azure Tenant Authentication===</span>"
[Console]::ResetColor()  
...
if ($connection) {
    ...
    Write-Host "<span style=`"color:#b10000 ;`">Failed to connect to Azure</span>"
}
...
Write-Output " "
Write-Host "<span style=`"color:#f6ff00;`">===Schedule $Runbook Runbook===</span>";[Console]::ResetColor()  
...
if ($groupMembers | Where-Object { $_.UserPrincipalName -eq $FinalTargetUPN }) {
    ...
    Write-Output " "
    if ($schedule) {
        Write-Host "<span style=`"color:#f6ff00;`">===Schedule Verification===</span>" 
